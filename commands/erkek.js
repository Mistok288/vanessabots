...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        let embed2 = new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('#640032').setTimestamp().setThumbnail(message.author.avatarURL).setFooter('🎄Haypnos');
        ...
        if (![""].some(ss => member.user.username.toLowerCase().includes(ss)) && member.user.discriminator !== "etiket varsa koyun yoksa 0000 yapın örnek : 0099 şeklinde yazın" && !message.guild.members.cache.get(member.id).roles.cache.has('viprolüİD' && 'boosterrolüİD')) {
        ...
        db.push(`isimler_${member.id}`, ` \`${name} | ${age}\` (<@&807331495630077973>)`);
